...
- **Example Response**:
  ...
  ```
- **Error Handling**:
  - If either `a` or `b` is missing or invalid, the API returns a `400 Bad Request` response with an error message.
...
To test the sum endpoint, you can use tools like `curl` or Postman. For example:
```bash
